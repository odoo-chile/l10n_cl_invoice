...
class SiiTaxCode(models.Model):
    ...
    def compute_all(self, price_unit, currency=None, quantity=1.0,
        ...
        RETURN: {
            ...
            'taxes': [{               # One dict for each tax in self and t
            heir children
                'id': int,
                'name': str,
                'amount': float,
        ...
        # TODO: Resolver la cantidad de variables locales y argumentos
        # (disminuir)
        if len(self) == 0:
...
class AccountMove(models.Model):
    ...
    def _get_document_data(self, cr, uid, ids, name, arg, context=None):
        """ TODO: cambiar la api y la cantidad e argumentos
        (name y arg no se usan, por ahora las dejo, porque no sé si se
        está llamando desde otro lado y si usa esos argumentos, pero al menos
        dentro del módulo no.)"""
        res = {}
...
gravados o exentas.'),
         ('2', 'Facturas de proveedores registrados fuera de \
plazo.'),
         ('3', 'Gastos rechazados.'),
...
class account_journal(models.Model):
    ...
    journal_document_class_ids = new_fields.One2many(
        'account.journal.sii_document_class', 'journal_id', 'Documents Class',)
    ...
    journal_activities_ids = new_fields.Many2many(
        ...
        string='Journal Turns', help="""Select the turns you want to invoice
in this Journal""")
        ...
        try:
            if 'purchase' in self.type:
                ...
                for turn in self.journal_activities_ids:
                    _logger.info('turn {}'.format(turn.vat_affected))
    ...
    def _check_company_id(self):
        if self.point_of_sale_id and self.point_of_sale_id.company_id \
                != self.company_id:
